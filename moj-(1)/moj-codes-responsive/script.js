...
// ===== Nav highlight =====
const sections = ['home','services','how','availability','contact'].map(id => document.getElementById(id)).filter(Boolean);
...
contactForm?.addEventListener('submit', (e) => {
  ...
  if (!data.name || !data.phone || !emailOk || !data.subject || !data.message) {
    contactNote.textContent = 'يرجى تعبئة جميع الحقول والتأكد من صحة البريد.';
    contactNote.classList.add('error');
    return;
  }
  // Build a mailto fallback so the message actually reaches info@moj.sa
  const body = `الاسم: ${data.name}%0Aالجوال: ${data.phone}%0Aالبريد: ${data.email}%0A%0A${encodeURIComponent(data.message)}`;
...
signupForm?.addEventListener('submit', (e) => {
  ...
  const emailOk = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email || '');
  if (!emailOk || !data.phone || !data.role || !data.agree) {
    signupNote.textContent = 'يرجى تعبئة البريد والجوال، واختيار نوع التسجيل، والموافقة على الشروط.';
    signupNote.classList.add('error');
    return;
  }
  const roleLabel = data.role === 'seller' ? 'بائع' : 'مشترك';
  const body = `البريد: ${data.email}%0Aالجوال: 966${data.phone}%0Anوع التسجيل: ${roleLabel}`;
